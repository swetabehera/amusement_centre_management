    ...
    if (token) {
      restApp
        .authenticate(
          ...
          {
            query: {
              $eager: '[clients, clientPermissions]',
            },
          },
        )
        .then((response) => {
          const { user, accessToken } = response;
          if (user.clients[0]?.status === 2) {
          ...
          if (
            ...
            user.clientPermissions.length > 0 &&
            user.clientPermissions.filter((e) => e.metaName === 'client_login').length > 0
          ) {
